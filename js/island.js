...
(function () {
  ...
  const yourDates = m.yourDates ? `<div class="yourdates"><h5>Your dates · ${esc(HM.tripRangeLabel())}</h5><p>${esc(m.yourDates)}</p></div>` : "";
  const overview = `<section class="overview" aria-label="Overview">
    <div class="ov-top">
      <div><p class="short">${esc(isl.short)}</p><div class="unique"><h5>What makes ${esc(isl.name)} different</h5><p>${esc(isl.unique)}</p></div>${yourDates}</div>
  ...
  app.innerHTML = `<section class="ihero"><div class="hero-art">${UI.art(isl, { huts: isl.cost >= 4 })}</div>
    ...
    <main class="wrap">${overview}
      <div class="explore-head"><h2>Explore ${esc(isl.name)}</h2>
        <div class="seg big" id="view" role="tablist"><button role="tab" type="button" data-view="acts">Activities <small id="n-acts"></small></button><button role="tab" type="button" data-view="stay">Places to stay <small id="n-stay"></small></button><button role="tab" type="button" data-view="eat">Restaurants <small id="n-eat"></small></button></div></div>
      <div id="panel"></div>
  ...
  document.getElementById("n-eat").textContent = isl.eats.length;

  /* ---------- panels ---------- */
  const view = { v: location.hash === "#stay" ? "stay" : location.hash === "#eat" ? "eat" : "acts" };
  const AF = { q: "", tags: new Set(), mode: "any", tier: new Set(), price: "", dur: "", mine: "all", sort: "pop", hideoff: false, hideplaced: false };
  const SF = { q: "", type: new Set(), tags: new Set(), price: "", meals: "", mine: "all", sort: "price", hideplaced: false };
  const EF = { q: "", type: new Set(), tags: new Set(), tier: new Set(), price: "", meal: "", mine: "all", sort: "known", hideplaced: false };
  const FS = () => (view.v === "acts" ? AF : view.v === "stay" ? SF : EF);
  const chk = (name, label, on) => `<label class="check"><input type="checkbox" data-f="${name}" ${on ? "checked" : ""}> ${label}</label>`;
    ...
    if (view.v === "acts") {
      ...
      panel.innerHTML = `<div class="toolbar">
        ...
        <div class="tb-row"><span class="tb-label">Price</span>${sel("price", [["", "Any price"], ["0", "Free / included"], ["1", "$ (under $40 pp)"], ["2", "$$ ($40–120)"], ["3", "$$$ ($120–250)"], ["4", "$$$$ ($250+)"]], AF.price)}
          ...
          <span class="tb-label">Sort</span>${sel("sort", [["pop", "Most popular"], ["gems", "Hidden gems first"], ["plo", "Price: low → high"], ["phi", "Price: high → low"], ["dur", "Shortest first"], ["az", "A → Z"]], AF.sort)}</div>
        <div class="tb-row">${chk("hideoff", "Hide off-season for my dates", AF.hideoff)}${chk("hideplaced", "Hide what's already in my itinerary", AF.hideplaced)}<span class="legend">Season badges compare each activity with your trip dates (${esc(HM.tripRangeLabel())}).</span></div></div>
        <p class="count-line" id="count"></p><div class="acts" id="list"></div>`;
    } else if (view.v === "eat") {
      const ty = {}, tg = {}, tr = {}; isl.eats.forEach((e) => { ty[e.type] = (ty[e.type] || 0) + 1; tr[e.tier] = (tr[e.tier] || 0) + 1; e.tags.forEach((t) => (tg[t] = (tg[t] || 0) + 1)); });
      panel.innerHTML = `<div class="toolbar">
        <div class="tb-row"><label class="search">${UI.icons.search}<input data-f="q" type="search" placeholder="Search ${esc(isl.name)} restaurants, dishes, areas…" value="${esc(EF.q)}"></label>${mineSeg(EF.mine)}</div>
        <div class="tb-row"><span class="tb-label">Type</span><div class="chips" data-f="tier">${Object.keys(HM.TIERS).map((t) => `<button type="button" class="chip" data-v="${t}" aria-pressed="${EF.tier.has(t)}">${HM.TIERS[t]} <small>${tr[t] || 0}</small></button>`).join("")}</div>
          <div class="chips" data-f="type">${HM.EAT_TYPES.filter((t) => ty[t]).map((t) => `<button type="button" class="chip" data-v="${esc(t)}" aria-pressed="${EF.type.has(t)}">${esc(t)} <small>${ty[t]}</small></button>`).join("")}</div></div>
        <div class="tb-row"><span class="tb-label">Style</span><div class="chips" data-f="tags">${Object.keys(HM.EAT_TAGS).filter((t) => tg[t]).sort((a, b) => tg[b] - tg[a]).map((t) => `<button type="button" class="chip" data-v="${t}" aria-pressed="${EF.tags.has(t)}">${HM.EAT_TAGS[t]} <small>${tg[t]}</small></button>`).join("")}</div></div>
        <div class="tb-row"><span class="tb-label">Meal</span>${sel("meal", [["", "Any meal"], ["b", "Breakfast"], ["l", "Lunch"], ["d", "Dinner"]], EF.meal)}
          <span class="tb-label">Per person</span>${sel("price", [["", "Any price"], ["1", "$ (under $20)"], ["2", "$$ ($20–50)"], ["3", "$$$ ($50–100)"], ["4", "$$$$ ($100+)"]], EF.price)}
          <span class="tb-label">Sort</span>${sel("sort", [["known", "Best known first"], ["gems", "Hidden gems first"], ["plo", "Price: low → high"], ["phi", "Price: high → low"], ["az", "A → Z"]], EF.sort)}</div>
        <div class="tb-row">${chk("hideplaced", "Hide what's already in my itinerary", EF.hideplaced)}<span class="legend">Prices are a typical meal per person. The itinerary assumes you eat hotel-included meals at the hotel.</span></div></div>
        <p class="count-line" id="count"></p><div class="acts" id="list"></div>`;
    } else {
      ...
      panel.innerHTML = `<div class="toolbar">
        ...
        <div class="tb-row"><span class="tb-label">Nightly rate</span>${sel("price", [["", "Any rate"], ["1", "$ (up to $200)"], ["2", "$$ ($200–450)"], ["3", "$$$ ($450–1,000)"], ["4", "$$$$ ($1,000+)"]], SF.price)}
          <span class="tb-label">Meals</span>${sel("meals", [["", "Any"], ["any", "Includes any meal"], ["b", "Includes breakfast"], ["d", "Includes dinner"], ["bld", "Full board (all meals)"]], SF.meals)}
          <span class="tb-label">Sort</span>${sel("sort", [["price", "Price: low → high"], ["phi", "Price: high → low"], ["az", "A → Z"]], SF.sort)}</div>
        <div class="tb-row">${chk("hideplaced", "Hide places I've already booked in the itinerary", SF.hideplaced)}</div></div>
        <p class="count-line" id="count"></p><div class="acts" id="list"></div>`;
    }
    renderList();
  }
  ...
  function renderList() {
    ...
    if (view.v === "acts") {
      let r = isl.activities.filter((a) => {
        ...
        if (AF.hideoff && HM.season(a.mo) === "out") return false;
        if (AF.hideplaced && HM.placedDays("a", a.id).length) return false;
      ...
      const tierRank = { C: 0, U: 1, H: 2 };
      let r = isl.eats.filter((e) => {
        if (EF.q && !(e.name + " " + e.desc + " " + e.where + " " + e.type + " " + e.tags.map((t) => HM.EAT_TAGS[t]).join(" ")).toLowerCase().includes(EF.q.toLowerCase())) return false;
        if (EF.type.size && !EF.type.has(e.type)) return false;
        if (EF.tier.size && !EF.tier.has(e.tier)) return false;
        if (EF.tags.size && ![...EF.tags].some((t) => e.tags.includes(t))) return false;
        if (EF.meal && !e.meals.includes(EF.meal)) return false;
        if (EF.price !== "" && HM.eatLevel(e.pp) !== +EF.price) return false;
        if (EF.hideplaced && HM.placedDays("e", e.id).length) return false;
        return mineOk(EF.mine, "r:" + e.id);
      });
      r.sort((a, b) => EF.sort === "known" ? tierRank[a.tier] - tierRank[b.tier] || a.pp - b.pp : EF.sort === "gems" ? tierRank[b.tier] - tierRank[a.tier] || a.pp - b.pp : EF.sort === "plo" ? a.pp - b.pp : EF.sort === "phi" ? b.pp - a.pp : a.name.localeCompare(b.name));
      cnt.innerHTML = `Showing <b>${r.length}</b> of ${isl.eats.length} restaurants · prices are per person for a typical meal`;
      listEl.innerHTML = r.length ? r.map((e) => UI.eatCard(e)).join("") : `<div class="empty"><h3>${isl.eats.length ? "No restaurants match those filters" : "Restaurant research for this island is on the way"}</h3></div>`;
    } else {
      let r = isl.lodging.filter((s) => {
        ...
        if (SF.meals) { const m = s.meals || ""; if (SF.meals === "any" ? !m : SF.meals === "bld" ? m.length < 3 : !m.includes(SF.meals)) return false; }
        if (SF.hideplaced && HM.placedDays("s", s.id).length) return false;
  ...
  /* ---------- events ---------- */
  document.getElementById("view").addEventListener("click", (e) => { const b = e.target.closest("button"); if (!b) return; view.v = b.dataset.view; history.replaceState(null, "", view.v === "acts" ? location.pathname + location.search : "#" + view.v); shell(); });
  const panel = document.getElementById("panel");
  panel.addEventListener("click", (e) => {
    const b = e.target.closest("button[data-v]"); if (!b) return;
    const grp = b.closest("[data-f]"), F = FS(), k = grp.dataset.f;
  ...
  const onInput = (e) => { const t = e.target, k = t.dataset && t.dataset.f; if (!k) return; FS()[k] = t.type === "checkbox" ? t.checked : t.value; renderList(); };
  panel.addEventListener("input", onInput); panel.addEventListener("change", onInput);
  document.addEventListener("hm:rating", (e) => { if (e.detail.key.startsWith("i:")) return; if (FS().mine !== "all") renderList(); });
  document.addEventListener("hm:itin", () => { if (FS().hideplaced || FS().hideoff) renderList(); });
  document.addEventListener("hm:sync", () => renderList());
  shell();
})();
